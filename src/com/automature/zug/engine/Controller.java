...
//    private Hashtable<String, List<BusinessLayer.Variable>> TestCaseVariableValue = new Hashtable<String, List<BusinessLayer.Variable>>();
    ...
    public static String JavaVersion = "", JavaHome = "", JavaCompiler = "", JavaLibraryPath = "";
...
//This function prints the Config
    ...
    private void printConfig() {
        ...
        message("Zug is running in Java Version: " + JavaVersion + "\nJava Maximum Memory Size: " + Utility.getMaxJVMMemorySize(jvm));
...
//                + TopologySet.length); // TODO Discuss the way of saving the details through DAVOS ...
...
////        }
        ...
        for (int i = 0; i < executedTestCase.length; i++) {
            ...
            if (StringUtils.isNotBlank(testSuiteId)) {
                testCaseResult.set_testSuiteId(Integer.valueOf(testSuiteId).intValue());
            }
//            else
//                testCaseResult.set_testSuiteId(2012);
            ...
            if (testCycleId == null) {
                ...
                Log.Debug("Controller/SaveTestCaseResultEveryTime : TestCycle=" + testCycleId);
                // davosclient.testExecutionDetails_write(testCaseResult.get_testCaseId(), testCycleId, testCaseResult.get_status(),testCaseResult.get_buildNo(), new Integer(testCaseResult.get_testExecution_Time()).toString(),testCaseResult.get_performanceExecutionDetailTable().get_performanceExecutionDetailId() , testCaseResult.get_executionDate().toString(), testSuitName);
...
//    private boolean checkIfMVM(TestCase testcase, String value) throws Exception {
//        message("Controller/checkIfMVM: Method called using parameters " + testcase.testCaseID + "  " + value + "\n Size of Action " + testcase.actions.size());
...
//            if (single_action.actionActualArguments.size() == single_action.actionArguments.size()) {
//                for (int i = 0; i < single_action.actionActualArguments.size(); i++) {
//                    String purevalues = GetTheActualValue(single_action.actionArguments.get(i));
//                    if (value.trim().equals(purevalues.trim())) {
//                        message("The values are similar " + purevalues);
//                        message("The value is MVM without check " + single_action.actionActualArguments.get(i));
//                        if (single_action.actionActualArguments.get(i).startsWith("$$")) {
//                            checkMVM = true;
//                            message("The value is MVM " + single_action.actionActualArguments.get(i));
//                            break;
//                        }
//                        //TODO check = for named arguments
//                        if (single_action.actionActualArguments.get(i).contains("=")) {
//                            String[] split_actual_arg = Excel.SplitOnFirstEquals(single_action.actionActualArguments.get(i));
...
////                            checkMVM = false;
////                            message("The value not MVM " + single_action.actionActualArguments.get(i));
////                        }
//
//                    } else {
//                        continue;
...
//            }
//
//            Verification[] allVerificationInTestCase = new Verification[single_action.verification.size()];
...
//                if (single_verification.verificationActualArguments.size() == single_verification.verificationArguments.size()) {
//                    for (int i = 0; i < single_verification.verificationActualArguments.size(); ++i) {
//                        String purevalues = GetTheActualValue(single_verification.verificationArguments.get(i));
//                        if (value.trim().equals(purevalues.trim())) {
//                            // message("The values are verify similar " + purevalues);
//                            if (single_verification.verificationActualArguments.get(i).startsWith("$$")) {
//                                checkInVrifiMVM = true;
//                                //  message("The value is verify MVM " + single_verification.verificationActualArguments.get(i));
//                                break;
//                            }
//                            //TODO check = for named arguments
//                            if (single_verification.verificationActualArguments.get(i).contains("=")) {
//                                String[] split_actual_arg = Excel.SplitOnFirstEquals(single_verification.verificationActualArguments.get(i));
//                                if (split_actual_arg[1].startsWith("$$")) {
//                                    checkInVrifiMVM = true;
//                                    // message("The value is verify MVM = " + single_verification.verificationActualArguments.get(i));
//                                    break;
...
//    }
     ...
     */
    @Deprecated
    private boolean checkIfMVM(HashMap<String, String> mvm_map, String value) throws Exception {
...
//                    isMVM = true;
                    //message("action value ?" + action.actionArguments.get(i));
//                } else {
//                    continue;
//                }
                }
...
//We can comeup with a data structure which rembers the value with the macro key. if no macro key containing "$$" Then no expansion
                //new function for checking the $$ or $ to expand the testcase
                count1++;
                //message("Comming to Cartesian Product "+isMVM);
                ////if ((tempVal.startsWith("{")) && (tempVal.endsWith("}"))) {
...
///TODO:this method is not used now.
    private void SaveTestSuite() throws Exception, DavosExecutionException {
        ...
        testSuiteId = davosclient.testSuite_write(productid, roleid, testSuitName, "Ready", "Automatic", "", "");
        message("SaveTestSuite::\tThe test suite date to be send " + readExcel.TestSuitName() + "\t" + readExcel.TestSuitRole() + "\t" + roleid + "This is a product\t" + productid);
...
//        }
//
//        topologyIDS = Utility.TrimStartEnd(topologyIDS, ',', 0);//TODO get the product ID from DAVOS for the TestPlan
//        topologyIDS = Utility.TrimStartEnd(topologyIDS, ',', 1);//TODO get the Role ID from DAVOS for testsuite rolename
//
...
//        // Harness Specific ContextVariable to store TopologySet ID
    ...
    public boolean ConnectToDavos() throws DavosExecutionException {
        ...
        try {
            ...
            message("Connecting to DAVOS --> URL: " + readExcel.DBHostName() + " User: " + readExcel.DBUserName());
            davosclient = new DavosClient(readExcel.DBHostName(), readExcel.DBUserName(), readExcel.DBUserPassword());
    ...
    private void RunTestCaseForMain(Object act) throws Exception, DavosExecutionException {
...
boolean testcasenotfound=false;
 ...
 boolean testcasenotpresent=false;
     ...
     *            DataClass of the TestCase.
     */
    private void RunTestCase(TestCase test1) throws Exception, DavosExecutionException {
        if (test1.automated == false) {
...
//
     ...
     * @param testcaseObj
     */
    private void RunExpandedTestCase(Object testcaseObj) throws Exception, DavosExecutionException {
...
//Getting Operating System Information
        ...
        mvmconfiguration = Utility.getMaxJVMMemorySize(Runtime.getRuntime()).split("\\.")[0];
