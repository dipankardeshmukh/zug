...
public class Spark extends ZugGui {
	...
	private void InitializeVariables(Excel readExcel) throws Exception ,Throwable{
		...
		if (StringUtils.isEmpty(opts.scriptLocation)) {
			...
			String testSuiteLoc=new File(Spark.readExcel.getXlsFilePath()).getParent();
		...
		if (opts.repeatDurationSpecified) {
			CreateContextVariable("ZUG_TESTSUITE_TIMEOUT="
					+ ((int)(opts.repeatDurationLong / 1000) + readExcel
							.TESTPLAN_TIMEOUT()));
...
//		reporter=null;
	...
	static void showAtomPerformance(){
		...
		try{
			...
			Spark.message(String.format("%-55s%-5s %-8s %-5s","Atom Name","Min.","Avg.","Max.\n"));
			while(itTm.hasNext()){
				if(noOfItem==10){
					break;
				}
				...
				for(Object obj:l){
					...
					List<String> al=Spark.atomPerformance.get(obj.toString());
					Collections.sort(al);	
					Spark.message(String.format("%-55s%-5d %-5.2f  %-5d", obj,al.get(0),n,al.get(al.size()-1)));
					noOfItem++;
				}
			}
			Spark.message("\n********************************************************************************\n");
		}catch(Exception e){
			Spark.message("Controller/showAtomPerforamance:Exception-"+e.getMessage());
		}
	}
		...
		if (testINI.reteriveXmlTagAttributeValue(inprocess_jar_xml_tag_path,
			...
			for (String package_names : testINI.reteriveXmlTagAttributeValue(
				...
				if (StringUtils.isNotBlank(package_names)
					...
					if (ai.native_flag) {
						// Spark.message
						//System.out.println("This is native Dll Calling");
					} else if (ai.com_flag) {
						// Spark.message
						//System.out.println("Com Jacob Calling");
					} else {
						// Spark.message
						//System.out.println("invoking jar instance "+package_names);
						try{
					...
					System.out.println("[Warning] Spark.ini contains blank inprocess package definition. Please refer to the readme.txt or Zug User Manual");
				}
			}

			if (errorSet.size() > 0) {
				System.err.println(String
						.format("[Warning] %s has duplicate and incorrect for inprocess packages xml definition: 'language' attribute is not defined in Spark.ini",
								errorSet));
			}
		} else {
			System.out.println("No Inprocess Jar definition found in Spark.ini with proper Attribute definition for Tag");
		}
		if(guiFlag){
			System.out.println("\nInprocess packages Loaded.");
		}
	...
	public int getTestSuiteTimeout(){
		...
		try{
			...
			testSuiteTimeout=Spark.ReadContextVariable("ZUG_TESTSUITE_TIMEOUT");
...
//oldmain method
	...
	Exception, DavosExecutionException, MoleculeDefinitionException,Throwable {
		...
		Spark.readExcel = new Excel();

		final Spark controller = new Spark();
		String frameWork="";
		//	controller.CreateContextVariable("ZUG_LOGFILENAME="+ZUG_LOGFILENAME);

		ContextVar.setContextVar("ZUG_LOGFILENAME",Spark.ZUG_LOGFILENAME);
...
//			}catch(Exception e){e.printStackTrace();}
		...
		try {
			...
			Spark.message("Reading the TestCases Input Sheet  "
					+ opts.inputFile + ".\n");
			// Now reading the Excel object.

			Spark.readExcel.setXlsFilePath(opts.inputFile);

			Spark.CreateContextVariable("ZUG_BWD="
					+ new File(Spark.readExcel.getXlsFilePath())
					.getParent());
			// System.out.println("The base working directory "+ContextVar.getContextVar("ZUG_PWD"));
			if(Spark.errorOccured){
				clearStaticMembers();
				throw new Throwable();
			}
			Spark.readExcel.ReadExcel(opts.inputFile,
					opts.verificationSwitching, opts.compileMode);
			Spark.message("SUCCESSFULLY Read the TestCases Input Sheet "
					+ opts.inputFile);
			if(Spark.errorOccured){
				clearStaticMembers();
				throw new Throwable();
			...
			if (opts.compileMode) {
				if (Spark.readExcel.CompileTimeErrorMessage() != null) {
					Spark
					.message("\n******************** START: Compile Time Errors Present in the Test Design Sheet. ***************************");
					Spark.message(Spark.readExcel
			...
			if (opts.dbReporting == true) {
				...
				if(opts.dbReporting)
					if(Spark.guiFlag)
					reporter.testCycleClearTestCases(ZugguiController.controller.getProductId(),ZugguiController.controller.getTopologySetId(),ZugguiController.controller.getTestCycleId(),ZugguiController.controller.getBuildId(),testsuite.testSuitName);
					else
...
//			totalTestCases=controller.testsuite.testcases.length;
			...
			.message("\nTotal time taken to initialize : "
					+ Spark.initializationTime + " milliseconds.");
			Spark
			.message("\n******************************************************************************** ");
			if (!opts.verbose) {
				System.out
