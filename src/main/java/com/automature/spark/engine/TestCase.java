						...
						try {
							...
							Log.Error(String
									.format("Failure in sending request to %s.\n Call: testexecutiondetail/write \nRequests sent: testcaseid=%s,testcasedescription=%s,testcycleid=%s,testcaseresult=%s ..... topologysetid=%s \nError Message: %s",
											Spark.reporter.getClass().getSimpleName(),
											tData.testCaseID,
											tData.testcasedescription,
											Spark.opts.testCycleId, tData.testCaseStatus,
											Spark.opts.topologySetId, de.getMessage()));
							reportingError=true;

						}
						try {
							Spark.reporter.saveTestCaseVariables(
...
//			}
				...
				for (int i = 0; i < actions.length; i++) {
					...
					try{
						...
						if(Spark.guiFlag){
							...
							Spark.guiController.showRunningTestStep(action.lineNumber);
						}
					...
					if(Spark.opts.debugger){
						...
						ArrayList al=Spark.breakpoints.get(Excel.mainNameSpace);
					...
					// then Start with the new process...
						...
						for (int t = 0; t < ThreadPool.size(); ++t) {
							if (Spark.opts.debugMode) {
								...
								try{
									int stepTimeout = Integer.parseInt(Spark.ReadContextVariable("ZUG_TESTSTEP_TIMEOUT")) * 1000;
										...
										|| (action.step.endsWith("c")
												...
												.contains(action.step
														.substring(
																0,
																action.step
																.length() - 1)) && Spark.opts.doCleanupOnTimeout)) {
			...
			if(Spark.guiFlag){
				...
				Spark.guiController.updateTestCaseStatus(this.testCaseID, true);
			}
...
//					oldIndex+=1;
	...
	private TestCase checkWithCommandLineInput(TestCase tempcom) {
		...
		// manualTestCaseID);
		String cmdTestCases[] = Spark.opts.manualTestCaseID.split(",");
	...
	public String getIteratedValues(String valueToExpand){
		...
		if(valueToExpand.contains("..")){
			...
			Log.Debug(String.format(
					"TestCase/getContinuityValue : Length of stringsToCompare = %d",
					stringsToCompare.length));

			if (stringsToCompare.length < 2) {
				Log.Error(String
						.format("TestCase/getContinuityValue : [stringsToCompare.Length < 2] End of the Function  with valueToExpand = %s and return Value as = %s",
	...
	TestCase[] ExpandTestCase(boolean fromTestCase)
		...
		if (result != null) {
			...
			for (int p = 0; p < size; ++p) {
				...
				if (Spark.opts.manualTestCaseID.contains(",")) {
		...
		if (Spark.opts.excludeTestCaseID != null) {
			ArrayList<TestCase> removeTestCase = new ArrayList<TestCase>();
			if (Spark.opts.excludeTestCaseID.contains(",")) {

				String excludeTCID[] =Spark. opts.excludeTestCaseID.split(",");
				...
				for (TestCase tt : tempTestCases) {
					if (tt.testCaseID.equalsIgnoreCase(Spark.opts.excludeTestCaseID)) {
						removeTestCase.add(tt);
						break;
					}
				}
