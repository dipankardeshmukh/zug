...
import com.automature.spark.util.XMLWriter;
...
public class Excel {
	...
	public String DBHostName() throws Throwable{
		...
		try {
			...
			if (StringUtils.isBlank(dbHostName) && Spark.opts.dbReporting) {
	...
	public String DBName()throws Throwable {
		...
		try {
			...
			if (StringUtils.isBlank(dbName) && Spark.opts.dbReporting) {
	...
	public String DBUserName()throws Throwable {
		...
		try {
			...
			if (StringUtils.isBlank(dbUserName) && Spark.opts.dbReporting) {
	...
	public String DBUserPassword() throws Throwable{
		...
		try {
			...
			if (StringUtils.isBlank(dbUserPassword) && Spark.opts.dbReporting) {
		...
		while (set_key_iterator.hasNext()) {
			...
			if (xml_memory_size.equalsIgnoreCase(memorysize)
				...
				if (memory < xml_memory && memory > 1) {
					...
					checkOptimality = false;
					Log.Error("Excel/checkOptimalityForCartesianProduct: Spark.ini memory configuration is invalid: Not enough memory ");
				}
				break;
			}
	...
	public void ReadExcel(String inputFileName,
		...
		try {
			...
			_macroSheetHashTable.putAll(Spark.macrocommandlineinputs);
	...
	void GetKeyValuePair(Sheet worksheet,
		...
		if (sheetname.equals("macros")
				&& nameSpace != null
				&& (isMVC = Spark.macroColumnValue.containsKey(nameSpace)) != false) {
			//System.out.println(getMacroFileheaders(worksheet));
			try{
				value = Integer
						.parseInt(Spark.macroColumnValue.get(nameSpace));

			}catch(NumberFormatException nfe){
				Map<String,Integer> map=getMacroSheetheaders(worksheet);
				String str=Spark.macroColumnValue.get(nameSpace);
		...
		try {
			...
			while (rowIter.hasNext()){
				...
				if (sheetname.equals("config")
					...
					if (!Spark.opts.includeFlag) {
						Log.Debug("Command Line Executions switch Given\t"
								+ Spark.opts.includeFlag);
						String[] include_commandlist = Spark.opts.includeMolecules
								.split(",");
					...
					if (Spark.opts.includeFlag || !Spark.opts.includeFlag) {
