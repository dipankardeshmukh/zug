...
public class TestLinkReporter extends Reporter{
	...
	private void validateTestPlan()throws TestLinkAPIException{
		try{
			...
			Spark.message("TestLinkReporter:TestPlan "+testPlanName +" not found system will create a test plan");
			this.setBuild();
...
//			Date d=new Date();
		...
		Spark.message("TestLinkReporter:Build name was not provided or the build name doesn't exists System has created a new build: "+build.getName());
	}
