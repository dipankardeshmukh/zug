...
/* This function returns topology role for the specified topology id.
 * It assumes the topology id exists
 */
	public String getTopologyRole(int TopologyId) throws Exception{
		String tRoleID = null;
		try{		
			_transaction = _dbOperation.BeginTransaction(_transaction);
			DatabaseLayer.Topology topologyTable=new DatabaseLayer.Topology();
			Log.Debug(this.toString() + "/Topology: getTopologyRole() function called ");
			topologyTable.setTopologyId(TopologyId);
			
			Log.Debug(this.toString() + "/getTopologyRole : Select TopologyId entry  in Topology Table");
			List<IDataTable> selectedDataList = new ArrayList<IDataTable>();
			selectedDataList =	 (List<IDataTable>)_dbOperation.Select(topologyTable);
			Log.Debug(this.toString() + "/TopologyExist: Select query executed to check TopologyId exist or not.");
			if (selectedDataList == null)
            ...
            {
                topologyTable = (DatabaseLayer.Topology)selectedDataList.get(0);
                DatabaseLayer.Role TRoleObj = topologyTable.getRole();
                tRoleID = TRoleObj.getRoleId().toString();
