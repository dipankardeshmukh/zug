...
public class Controller extends Thread {
	...
	private void SaveTestCaseResult() throws Exception {
		...
		for (int i = 0; i < TopologySet.length; i++) {
			...
			testCasesExecutedMachine.set_topologyId(Integer
					.parseInt(TopologySet[i].topologyID));
			testCasesExecutedMachine
					.set_buildNumber(TopologySet[i].buildNumber);
			if (TopologySet[i].topologyRole
					.compareToIgnoreCase("Local Topology Name") == 0) {// need
																		// to
																		// change
																		// for
																		// test
			...
			try {
				Log.Debug("Controller/SaveTestCaseResultEveryTime : TestCycle="
						...
						+ (StringUtils.isBlank(testCycleId) ? "Null"
								: testCycle.TestCycleExist(Integer
										.parseInt(testCycleId))).toString());
					...
					for (int i = 0; i < action.actionArguments.size(); ++i) {
						if (multiValuedVariablePosition.containsKey(count)) {
							...
							if (fromTestCaseSheet == true) {
								...
								var.set_name(ReturnValue(action.actionActualArguments
										.get(i).toString()));
						...
						for (int i = 0; i < verification.verificationArguments
							...
							if (multiValuedVariablePosition.containsKey(count)) {
								...
								if (fromTestCaseSheet == true) {
									...
									BusinessLayer.Variable var = new BusinessLayer.Variable();
									var.set_name(ReturnValue(verification.verificationActualArguments
											.get(i).toString()));
									var.set_value(ReturnValue(tempTestCaseVar[count]));
...
////TODO I need to Check here what is going on with the Values
	...
	public boolean ValidateDatabaseEntries() {
		...
		try {
			if (!topologysetObj.TopologysetExist(Integer
					.parseInt(topologySetId))) {
		...
		try {
			...
			List<Integer> topologyIDs = topologysetObj
					.ReadTopologySetXref(Integer.parseInt(topologySetId));
		...
		for (int i = 0; i < TopologySet.length; i++) {
			...
			try {
				if (!topologyObj.TopologyExist(Integer
						.parseInt(TopologySet[i].topologyID))) {
					...
					TopologySet[i].topologyRole = topologyObj
							.getTopologyRole(Integer
									.parseInt(TopologySet[i].topologyID));
				}
			} catch (NumberFormatException e) {
	...
	private void SaveTestSuite() throws Exception {
		...
		// /get the TestPlan ID
		testplanUpdateData.testPlanData.setTestPlanId(Integer
				.parseInt(TestPlanId));

		BusinessLayer.TestPlan testplanObj = new BusinessLayer.TestPlan();

		// if this works, then possibly remove all the manual setters above
		testplanUpdateData.testPlanData = testplanObj.getTestPlanData(Integer
				.parseInt(TestPlanId));
		...
		for (int i = 0; i < TopologySet.length; i++) {
			...
			topologyDetail.set_topologyId(Integer
					.parseInt(TopologySet[i].topologyID));
			...
			while (repeatDurationLong > 1000 * 60) {
				...
				for (TestCase test : testcases) {
					...
					if (test.testCaseID.compareTo("init") != 0)
						if (StringUtils.isNotBlank(manualTestCaseID))
							if (!manualTestCaseID.contains(test.testCaseID
									.trim()))
								continue;
			...
			if (action.actionName.startsWith("@")) {
				...
				try {
					...
					String attributeNames = StringUtils.isBlank(((String) ht
							...
							: ((String) ht.get("attributenames"));
					String tagName = StringUtils.isBlank(((String) ht
							.get("tagnames"))) ? StringUtils.EMPTY
	...
	private void ExecuteCommand(String command, String arguments,
		...
		try {
			...
			String error = String
					.format("Exception raised while running Command %s.\n\tException Message is : %s",
	...
	private String DoSomeFineTuning(String variable, String threadID)
		...
		for (char varChar : variable.toCharArray()) {
			indexer++;
			Log.Debug(String
					.format("Controller/DoSomeFineTuning : Working at Indexes[%d]=%s in Variable %s.",
							indexer, varChar, variable));
			if (varChar == '%') {
				Log.Debug(String
						.format("Controller/DoSomeFineTuning : Indexes=%d in Variable %s.",
								indexer, variable));
				if (firstOccuranceOfPercentage >= 0) {
					secondOccuranceOfPercentage = indexer;
					Log.Debug(String
							.format("Controller/DoSomeFineTuning : secondOccuranceOfPercentage=%d in Variable %s.",
									indexer, variable));
					break;
				} else {
					firstOccuranceOfPercentage = indexer;
					Log.Debug(String
							.format("Controller/DoSomeFineTuning : firstOccuranceOfPercentage=%d in Variable %s.",
									indexer, variable));
				}
			}
		}
	...
	private void SaveTestCaseResultEveryTime(ExecutedTestCase tData)
		...
		testCycleData.setTestplan_id(Integer.parseInt(TestPlanId));
		Log.Debug(String
				.format("Controller/SaveTestCaseResultEveryTime : Saving TestCycle ID %s and Test TestPlan ID %s of Test Suite: %s.",
						testCycleId, TestPlanId, testSuitName));

		BusinessLayer.TopologySetResultData topologyResultData = new BusinessLayer.TopologySetResultData();

		Log.Debug(String
				.format("Controller/SaveTestCaseResultEveryTime : Number of Topology Set to add is %d",
						TopologySet.length));
		for (int i = 0; i < TopologySet.length; i++) {
			BusinessLayer.TopologyDetail testCasesExecutedMachine = new BusinessLayer.TopologyDetail();
			...
			testCasesExecutedMachine
					.set_buildNumber(TopologySet[i].buildNumber);
			if (TopologySet[i].topologyRole
					.compareToIgnoreCase("Local Topology Name") == 0) {
				testCasesExecutedMachine.set_role(testSuitRole);
				buildNumber = testCasesExecutedMachine.get_buildNumber();
			} else {
				testCasesExecutedMachine.set_role(TopologySet[i].topologyRole);
			...
			Log.Debug(String
					.format("Controller/SaveTestCaseResultEveryTime : TopologyName = %s BuildNumber = %s and  Role= %s will be saved.",
							testCasesExecutedMachine.get_topologyId(),
							testCasesExecutedMachine.get_buildNumber(),
							testCasesExecutedMachine.get_role()));
			topologyResultData.get_topologyDetailList().add(
		...
		Log.Debug(String
				.format("Controller/SaveTestCaseResultEveryTime : TestCaseId = %s TestSuiteId = %s Status = %s and Comments = %s will be saved.",
						testCaseResult.get_testCaseId(),
		...
		Log.Debug(String
				.format("Controller/SaveTestCaseResultEveryTime : Calling testCycle.Save method with TestCycle Data filled above."));

		if (testCycle == null)
			Log.Debug(String
					.format("Controller/SaveTestCaseResultEveryTime : TestCycle=Null"));
		else {
			Log.Debug(String.format(
			...
			try {
				Log.Debug(String
						.format("Controller/SaveTestCaseResultEveryTime : TestCycleExist= %s",
								StringUtils.isBlank(testCycleId) ? "Null"
										: testCycle.TestCycleExist(Integer
												.parseInt(testCycleId))));
			} catch (NumberFormatException e) {
	...
	private void ArchiveLog() {
		...
		try {
			...
			archiveLogLocation = frameworkMetaData
					.GetValue(BusinessLayer.Name.ArchiveLocation);
			...
			for (int i = 0; i < productLogFiles.length; i++) {
				...
				if (file.exists()) {
					...
					String destinationPath = productArchive + "\\" + fileName
							+ ".txt";
			...
			String zipTmpLocation = System.getenv("TEMP") + "\\"
					+ exactDateTime + ".zip";
			...
			if (Utility.deleteDirectory(new File(zugArchive)))
				zugArchivePath.mkdirs();
			Utility.copyFile(zipTmpLocation, zugArchive + "\\" + exactDateTime
					+ ".zip");
			...
			if (productArchivePath.list().length != 0) {
				String zipProductLocation = System.getenv("TEMP")
	...
	private void RunExpandedTestCase(Object testcaseObj) throws Exception {
		...
		try {
			...
			String failureReason = String
					.format("Status FAILED FOR TestCase ID %s. Exception MESSAGE IS : \n%s .",
							test.testCaseID, ex.getMessage());
			// String failureReason =
			// String.format("Status FAILED FOR TestCase ID %s. Exception MESSAGE IS : %s .",
			// test.testCaseID,(String)errorMessageDuringTestCaseExecution.get(test.parentTestCaseID));
	...
	public static void main(String[] args) throws InterruptedException,
		...
		if (args.length > 1) {
			for (String arg : args) {
				...
				if (arg.equalsIgnoreCase("-nyon")) {
					nyonserver = true;
					System.out.println("This is the Nyon-Server executions\t"
							+ nyonserver);
				}
		...
		try {
			...
			if (controller.opts.isVersionRequest()) {
				try {
					...
					if (licenseValid.matchMac() == false) {
						controller
								.message("Controller/Main: Please get a valid license for your machine");
						System.exit(1);
					}
					if (licenseValid.isDateValid() == false) {
						controller
								.message("Controller/Main: The License of ZUG has expired. Please renew. \n\tVisit www.automature.com");
						System.exit(1);
					}
					...
					controller
							.message("Controller/main : Failed to validate your License copy");
			...
			controller
					.message("Controller/Main : Failed to Validating Command Line Arguments, exiting "
		...
		try {
			...
			controller.message("Automation started for "
					+ licenseValid.userInfo.companyName);
		...
		try {
			...
			if (Controller.dbReporting == true) {
				...
				// / Even if the DB reporting is FALSE, still we should actually
				// show the statistics.
