...
public class OutProcessAtom implements Atom{
	...
	private void ExecuteCommand(String command, ArrayList<String> argument,
		...
		if (command.contains(".")) {
			...
			if (Spark.fileExtensionSupport
					.containsKey(commandNameExtension[1].toLowerCase())) {
				String[] commandList = Spark.fileExtensionSupport
		...
		try {
			...
			while ((primitiveStreams = stdInput.readLine()) != null) {
				Log.Debug("OutProcessAtom/ExecuteCommand : [AtomLog/" + FileName
						+ "] - " + primitiveStreams);
				if(Spark.opts.debugMode||Spark.opts.debugger){
					Spark.message("OUTPUT->\t"+primitiveStreams);
				}
			}
			...
			if (Spark.opts.debugMode
					|| (Spark.opts.doCleanupOnTimeout && step.endsWith("c"))) {
		...
		try {
				...
				if (StringUtils.isNotBlank(prototype.InProcessDllName)) {
					...
					if (type.equalsIgnoreCase("action")) {
						...
						ExecuteCommand(command, arg,
								Spark.opts.scriptLocation, user,
			...
			if(Spark.opts.showTime){
				...
				if(Spark.atomPerformance.containsKey(action.name.toLowerCase())){
					...
					al.add(hft.Duration());
					Spark.atomPerformance.put(action.name.toLowerCase(),al);
				}
				Spark
