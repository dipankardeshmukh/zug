...
public class ProgramOptions {
	...
	public static void populateMacroColumnValue(String str){
		...
		for(String temp:fileArr){
			...
			Spark.macroColumnValue.put(nameSpace, columnValue.toLowerCase());
		}    	
	}
			 ...
			 if (opt.toLowerCase().contains("-macrofile")) {
				 ...
				 try {
					 ...
					 while ((macro = br.readLine()) != null) {
						...
						//	 Excel ee = new Excel();
							 macroKey = Spark.readExcel.AppendNamespace(macroKey, namespaces);
							 if ((macroValue.contains("$$") || macroValue
								 ...
								 macroValue = Spark.readExcel.ExpandMacrosValue(macroValue,
										 macroKey, namespaces);
							 }
							 // ht.put(macrokey, macrovalue);
							 // Creating the command line Macro hashmap
							 Spark.macrocommandlineinputs.put(macroKey,
									 macroValue);
				 ...
				 if (temp.length == 2) {
					...
					// Excel ee = new Excel();
					 ...
					 if ((macrovalue.contains("$$") || macrovalue.contains("$")) && (macrovalue.startsWith("{") && macrovalue.endsWith("}"))) {
						 ...
						 macrovalue = Spark.readExcel.ExpandMacrosValue(macrovalue, macrokey, namespaces);
					 }
					 //ht.put(macrokey, macrovalue);
					 //Creating the command line Macro hashmap 
					 Spark.macrocommandlineinputs.put(macrokey, macrovalue);
					 //System.out.println("The Command line changing "+Controller.macrocommandlineinputs);
